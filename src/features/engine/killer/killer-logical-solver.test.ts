...
describe('KillerLogicalSolver — Tier 1', () => {
  ...
  it('grades and soundly solves a puzzle that genuinely requires Tier 4', () => {
    // Tier 4 = classic advanced + extreme strategies (X-Wing … AIC). Find a puzzle only Tier 4
    // finishes, proving those strategies are wired in, necessary, and sound over cage candidates.
    let found = false;
    for (let seed = 1; seed <= 200 && !found; seed++) {
      const puzzle = generateKillerSudoku('hard', {
        solution: SOL9,
        rng: mulberry32(seed * 100 + 4),
        maxSize: 4,
      });
      const ls = new KillerLogicalSolver(puzzle.cages, 9);
      const result = ls.solve();
      if (result.solved && result.hardestTier === 4) {
        found = true;
        expect(unsoundPlacements(ls.grid, SOL9)).toBe(0);
        expect(ls.grid).toEqual(SOL9);
      }
