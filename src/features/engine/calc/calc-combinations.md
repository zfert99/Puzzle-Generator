# Calcudoku Cage-Combination Tables (`calc-combinations.ts`)

The arithmetic core of Calcudoku. Answers the one question every Calcudoku deduction depends on:
for a cage of `size` cells with operator `op` and target `target` on an N×N grid, which
**multisets** of digits produce the target?

## Multisets, not sets (the #1 porting bug)

Calcudoku permits a digit to repeat within a cage (as long as the repeats don't share a row/column),
...
|---|---|---|---|
| `calcUnionMask` | **upper** bound | digits in ANY valid multiset | geometry only removes multisets, so the true usable-digit set is a subset — never wrongly *excludes* a digit (safe for candidate pruning), but over-counts for line cages |
| `calcGuaranteedMask` | **lower** bound | digits in EVERY valid multiset | removing multisets can only make a digit *more* guaranteed — a digit guaranteed over all stays guaranteed over the valid subset (safe to eliminate on) |
...
```text
calcCombosFor('mul', 4, 6, 6) → [[1,1,1,6],[1,1,2,3]]   the classic 6× four-cell case
calcCombosFor('add', 2, 4, 6) → [[1,3],[2,2]]           note the {2,2} repeat multiset
calcCombosFor('sub', 2, 3, 4) → [[1,4]]                 3− on 4×4
calcCombosFor('div', 2, 3, 4) → [[1,3]]                 3÷ on 4×4
calcCombosFor('div', 2, 1, 6) → []                      1÷ impossible (collinear two-cell)
calcGuaranteedMask('mul', 4, 6, 6) → {1}               both 6× multisets contain a 1
```
