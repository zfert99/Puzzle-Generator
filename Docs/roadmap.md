...
in `.mcp.json` as `sudoku-hint`), a one-hint agent on the Anthropic SDK (`npm run hint:agent`),
and an eval harness (`npm run hint:eval`) that scores validity, strategy label, leak and refusal
...
- **Stage 1 — make the gate physical:** 🚧 *partly done, August 2026.*
  ...
  - ⏳ **A doc-reference guard** — a CI check that fails when live source cites a `Docs/*.md` path
    that does not resolve. Section 7 already says to grep `src/` before archiving a doc; nothing
    enforces it, and the one time it was skipped nothing noticed (`killer-6x6-implementation-plan.md`,
    August 2026 — see its banner). Prototyped and set aside rather than bundled into that doc fix;
    the sketch had three known holes worth closing first: it passed silently when the pathspec
    matched nothing, flagged untracked-but-present docs as missing, and validated only the
    `Docs/…` tail of a path, so wrong-depth relative links in mirrored `src/**/*.md` docs slipped
    through.
