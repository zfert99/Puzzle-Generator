...
> **Status:** 🚧 In Progress — **engine K0–K4 + all K5 surfaces (play/PDF/hub/daily) + a measured difficulty rebalance + Keisan minis in the daily rotation + the full 5-tier 9×9 ladder (K7a–K7d)** shipped; Keisan is playable, printable, discoverable, and in the daily rotation at 4×4/6×6/9×9, with a **5-tier 9×9 ladder** (easy/medium/hard/**expert**/**extreme**) at parity with Classic/Killer ([walkthrough](keisan-walkthrough.md)). **K7 was re-sliced** after a 9×9 de-risk found maxSize-5/T5 infeasible and the solver capped at ~T2: **K7a** (3-tier givens-gradient) → **K7b** (bounded-recursion "T5", the keen.c transplant — measured that guess *depth* never exceeds 1) → **K7c** (Expert = needs a depth-1 Nishio guess) → **K7d** (Extreme = needs *many* Nishio steps — Slice-0 instrumentation found the guess-step *count* is a monotone difficulty axis, ~28× solve-time spread, so the research's "Option 2" won with no solver expansion; the [option-4 technique](research/compass_artifact_wf-bb40e383-544c-5bfa-a4d0-a3b6202da609_text_markdown.md) build was not needed). **Next: K6** No-Op / Mystery. See [keisan-9x9-feasibility-findings.md](research/keisan-9x9-feasibility-findings.md) + the [honest-ladder research](research/compass_artifact_wf-feb5af89-67a1-51e8-bf2f-f348f76adfdd_text_markdown.md). Displayed as **Keisan** (internal slug `calc`). Full plan: [kenken-implementation-plan.md](kenken-implementation-plan.md), reviewed twice (reuse audit + [external plan review](research/kenken-plan-review.md), GREEN) + a [difficulty-calibration](research/kenken-difficulty-calibration.md) pass
