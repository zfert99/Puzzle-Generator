...
Once these symmetry operations are collapsed into equivalence classes, the 288 valid Shidoku grids are reduced to merely two "essentially different" canonical base grids. This profound reduction indicates that every valid 4x4 Sudoku *solution* is simply a symmetrical transformation or digit relabeling of one of these two foundational architectures. Separately, if you count all *puzzles* (partially-filled grids with a unique solution, at any clue count) rather than completed grids, the total is 13,581,312 valid puzzles, of which 85,632 are minimal (irreducible).
...
The modern generation of Mini Sudoku relies almost entirely on plain backtracking search — the 4x4 and 6x6 state spaces are small enough (288 and 28.2 million completed grids respectively) that a bitmask backtracking solver finishes in well under a millisecond, making heavier machinery like general-purpose SMT solvers (e.g., Z3) unnecessary overhead for this scale, even though they *could* technically express the constraints. The standard computational pipeline for generating a well-posed Rokudoku follows a strict procedural sequence. First, a terminal state is achieved by generating a completely filled, valid 6x6 grid. This is often accomplished rapidly using Las Vegas algorithms, which introduce digits randomly until a constraint conflict occurs, at which point a deterministic depth-first search (backtracking) corrects the conflict and completes the board.
...
The 8-clue minimum was established by Ed Russell in 2006 through exhaustive computer search (no proper 7-clue 6x6 grid exists). In practice, an 8-clue grid is a statistical rarity — naive random-deletion generators (dig-hole without deliberately targeting minimal configurations) far more often stabilize at 9–12 clues, since hitting every unavoidable set down to the theoretical floor requires the digger to get lucky or search deliberately for it, not just delete randomly until stuck.

For context on how these critical sets scale with grid dimensions: the minimum for a standard 9x9 Sudoku is 17, a threshold rigorously *proven* via exhaustive supercomputer search by McGuire, Tugemann, and Civario (2012/2014). Unlike the 4x4, 6x6, and 9x9 cases, the minimums for larger grids below are the smallest configurations *found so far* by exhaustive or heuristic search on community forums (chiefly the New Sudoku Players' Forum), not proven-optimal lower bounds — it remains an open question whether they can be improved: 14 clues for 8x8 (4x2 blocks, found by C. Lass, 2012), 22 clues for 10x10 (5x2 blocks), and 30 clues for 12x12 (4x3/3x4 blocks) or 32 clues for 12x12 with 6x2 blocks — the achievable minimum depends on the block shape, not just the grid size.

| Grid Size | Block Topology | Smallest Known / Proven Minimum Clues | Maximum Clues (Incomplete) |
| :--- | :--- | :--- | :--- |
| 4x4 | 2x2 | 4 (proven) | 12 |
| 5x5 | Pentomino | 6 (smallest known) | N/A |
| 6x6 | 2x3 | 8 (proven) | N/A |
| 8x8 | 4x2 | 14 (smallest known) | N/A |
| 9x9 | 3x3 | 17 (proven) | 77 |
...
To formalize difficulty ratings accurately, computational models employ specific metrics designed to simulate human cognitive bottlenecks rather than raw processing power. Two well-documented families dominate consumer software: **bottleneck grading** (used by Sudoku Explainer, sometimes called the "Serate" metric), where the puzzle's difficulty equals the single hardest technique required anywhere in the solve, and **cumulative grading** (used by HoDoKu), where every technique application throughout the whole solve path adds to a running score. These two philosophies can rank the same pair of puzzles differently, since one cares only about the hardest step and the other about the total workload — worth knowing before picking (or building) a difficulty engine, since it changes what "hard" means to your users.

A third, more academically rigorous approach is the Ercsey-Ravasz and Toroczkai model (2012), which treats Sudoku as a deterministic, continuous-time dynamical system inspired by analog computation and statistical physics. It measures the "chaos" — the escape rate from a chaotic transient — in the trajectory the system takes toward the solution, and this escape rate correlates strongly with how long human solvers take on the same puzzle. Separately, empirical work by Radek Pelánek (the "SiSuS" model referenced in the sibling sudoku-generation document) found that a *dependency*-based metric — how serialized versus parallel the required deductions are, rather than which named techniques are needed — correlates with real human solve times at roughly 0.88, rising to about 0.95 once Sudoku-specific technique identifiers are folded back in. The practical takeaway for a generator: raw clue count and technique checklists are weaker predictors of felt difficulty than the *shape* of the deduction path (how much of it is forced into one long chain versus several independent easy branches).
...
Relational constraints dictate how adjacent cells interact. In Kropki Sudoku, cells separated by a black dot must contain digits in a precise 2:1 ratio (e.g., 2 and 4, or 3 and 6), while cells separated by a white dot must contain consecutive digits (e.g., 4 and 5). The application of these dots is restrictive enough that solvers on the New Sudoku Players' Forum have found dot-only 6x6 grids (no given digits at all) with a unique solution using just 7 dots — though this is only the smallest configuration found so far, not a proven lower bound.
...
- Corrected the total 4x4 valid-puzzle count from 13,579,680 to the verified figure of 13,581,312 (verified against published enumeration data: 288 completed grids, 85,632 minimal puzzles, split 25,728 four-clue / 58,368 five-clue / 1,536 six-clue). The 85,632 minimal-puzzle count and the "36 essentially different minimal puzzles (13 with 4 clues)" claim were both verified as accurate and left unchanged.
- Verified as accurate and left unchanged: Rokudoku's 28,200,960 total completed grids and 49 essentially-different grids (Ed Russell & Frazer Jarvis), and the 8-clue proven minimum (Ed Russell, 2006, via exhaustive search showing no 7-clue 6x6 grid exists).
- Corrected the larger-grid minimum-clue table and surrounding text: only the 4x4, 6x6, and 9x9 minimums are rigorously *proven*. The 8x8 (14), 10x10 (22), and 12x12 values are the smallest configurations *found so far* by forum-driven search (New Sudoku Players' Forum), not proven lower bounds — flagged this explicitly. Also fixed the 12x12 figure: 32 clues applies to 6x2 blocks, but this document's own typology table uses 3x4/4x3 blocks, for which the smallest known is 30 clues, not 32.
- Softened the Kropki-Sudoku claim that 7 dots is the "theoretical minimum" for a clue-free 6x6 grid — this is the smallest configuration found on the solving forums, not a proven bound. Reworded accordingly.
- Removed an unverifiable, specific-sounding statistic ("0.2% of 300,000 sampled puzzles hit 8 clues, 88.7% stabilized at 10-12") that could not be traced to any real source.
- Replaced likely-invented named metrics ("Nishio Human Cycles metric," "Clause Length Distribution metric," and an unsupported "0.86 Spearman correlation" for an "Explainer Rating") with the well-documented, verifiable distinction between bottleneck grading (Sudoku Explainer) and cumulative grading (HoDoKu) — both covered in more depth in the sibling expert-sudoku-strategies document — plus the real, citable Pelánek/SiSuS correlation figures (0.88 rising to 0.95) referenced in the sibling sudoku-generation document, for consistency across the three files.
- Softened the claim that SMT solvers (Z3) are part of the "modern" mini-Sudoku generation pipeline — in practice, plain bitmask backtracking is what's actually used at this scale (4x4/6x6 state spaces solve in far under a millisecond), and SMT solvers are unnecessary overhead here. Reframed accordingly.
- Trimmed minor redundant phrasing in a few places without cutting technical content.
