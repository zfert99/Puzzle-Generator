...
- **(e) Rate-limit bypass — REAL, verify keying.** If Upstash/KV limits are keyed on client IP via `x-forwarded-for`, note that the header survives the proxy hop (Vercel appends the client IP), so per-IP limiting still works through the hub. But a **directly-reachable origin lets an attacker skip any hub-level protections** (e.g. WAF rules, headers) and hit auth/API endpoints straight. Re-locking the alias closes this.
